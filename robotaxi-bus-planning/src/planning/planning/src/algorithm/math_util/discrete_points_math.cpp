...
bool ComputeDiscretePathProfile(
  ...
  for (std::size_t i = 0; i < points_size; ++i) {
    ...
    AINFO_IF(FLAGS_enable_debug_speedplan)<<i<<", s = "<<accumulated_s->at(i)<<", kappa = "<<kappa;
    if (kappa > 0) {
      if (kappa > max_kappa) {
        max_kappa = kappa;
        max_index = i;
  ...
  AINFO_IF(FLAGS_enable_debug_motion)<<max_index<<", max_kappa = "<<max_kappa<<", "<<min_index<<", min_kappa = "<<min_kappa;
