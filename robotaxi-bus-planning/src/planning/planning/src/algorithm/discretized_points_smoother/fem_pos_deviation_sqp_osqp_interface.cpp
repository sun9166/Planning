/**
 * @file fem_pos_deviation_sqp_osqp_interface.cpp
 **/
...
void FemPosDeviationSqpOsqpInterface::CalculateKernel(
  ...
  // Three quadratic penalties are involved:(涉及到三个二次惩罚)
  // 1. Penalty x on distance between middle point and point by finite element
  // estimate;(基于有限元估计的中点到点距离的惩罚x)
  // 2. Penalty y on path length;(路径长度的惩罚y)
  // 3. Penalty z on difference between points and reference points(平滑后的点与参考点偏移量的惩罚z)

  // General formulation of P matrix is as below(with 6 points as an example):
  // I is a two by two identity matrix, X, Y, Z represents x * I, y * I, z * I (I是一个二乘二的单位矩阵，X，Y，Z代表X*I，Y*I，Z*I)
