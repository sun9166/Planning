/**
 * @file fem_pos_deviation_osqp_interface.cpp
 **/
...
bool FemPosDeviationOsqpInterface::Solve() {
  ...
  if (!CalculateKernel(&P_data, &P_indices, &P_indptr)) {
...
bool FemPosDeviationOsqpInterface::CalculateKernel(
    std::vector<c_float>* P_data, std::vector<c_int>* P_indices,
    std::vector<c_int>* P_indptr) {
  if (num_of_variables_ <= 4) {
    return false;
  }
  ...
  if (col_num != num_of_variables_) {
    AERROR<<"col_num != num_of_variables_!!!!!!!!!!!!!!!!!!";
    return false;
  }
