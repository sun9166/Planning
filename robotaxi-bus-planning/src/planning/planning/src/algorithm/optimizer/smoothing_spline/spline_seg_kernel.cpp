...
void SplineSegKernel::CalculateFx(const uint32_t num_params) {
  ...
  int kernel_fx_rows_size =  kernel_fx_.rows();
  for (int r = 0; r < kernel_fx_rows_size; ++r) {
    int kernel_fx_cols_num =kernel_fx_.cols();
    for (int c = 0; c < kernel_fx_cols_num; ++c) {
...
void SplineSegKernel::CalculateDerivative(const uint32_t num_params) {
  ...
  int kernel_derivative_rows_num = kernel_derivative_.rows();
  for (int r = 1; r < kernel_derivative_rows_num; ++r) {
    int kernel_derivative_cols_num = kernel_derivative_.cols();
    for (int c = 1; c < kernel_derivative_cols_num; ++c) {
...
void SplineSegKernel::CalculateSecondOrderDerivative(
  ...
  int kernel_second_order_derivative_rows_num = kernel_second_order_derivative_.rows();    
  for (int r = 2; r < kernel_second_order_derivative_rows_num; ++r) {
    int kernel_second_order_derivative_cols_num = kernel_second_order_derivative_.cols();
    for (int c = 2; c < kernel_second_order_derivative_cols_num; ++c) {
...
void SplineSegKernel::CalculateThirdOrderDerivative(const uint32_t num_params) {
  kernel_third_order_derivative_ =
      Eigen::MatrixXd::Zero(num_params, num_params);
  int kernel_third_order_derivative_rows_num = kernel_third_order_derivative_.rows();    
  for (int r = 3; r < kernel_third_order_derivative_rows_num; ++r) {
    int  kernel_third_order_derivative_cols_num =  kernel_third_order_derivative_.cols();
    for (int c = 3; c < kernel_third_order_derivative_cols_num; ++c) {
