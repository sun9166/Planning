...
bool Spline1dConstraint::AddBoundary(const std::vector<double>& x_coord,
  ...
  uint32_t filtered_lower_bound_size = filtered_lower_bound.size();
  for (uint32_t i = 0; i < filtered_lower_bound_size; ++i) {
  ...
  uint32_t filtered_upper_bound_size = filtered_upper_bound.size();
  for (uint32_t i = 0; i < filtered_upper_bound_size; ++i) {
...
bool Spline1dConstraint::AddThirdDerivativeBoundary(
  ...
  if (!FilterConstraints(x_coord, lower_bound, upper_bound,
                         &filtered_lower_bound_x, &filtered_lower_bound,
                         &filtered_upper_bound_x, &filtered_upper_bound)) {
    AWARN_IF(FLAGS_enable_debug_motion)<< "Fail to filter constraints.";
    return false;
  }

  if (x_knots_.size() < 2) {
    AWARN_IF(FLAGS_enable_debug_motion)<< "x_konts size cannot be < 2.";
    return false;
  }
...
bool Spline1dConstraint::AddMonotoneInequalityConstraint(
  ...
  uint32_t x_coord_size = x_coord.size();
  for (uint32_t i = 1; i < x_coord_size; ++i) {
    ...
    uint32_t cur_coef_size = cur_coef.size();
    if (cur_spline_index == prev_spline_index) {
      
      for (uint32_t j = 0; j < cur_coef_size; ++j) {
...
bool Spline1dConstraint::FilterConstraints(
  ...
  uint32_t lower_bound_size = lower_bound.size();
  for (uint32_t i = 0; i < lower_bound_size; ++i) {
  ...
  uint32_t upper_bound_size = upper_bound.size();
  for (uint32_t i = 0; i < upper_bound_size; ++i) {
