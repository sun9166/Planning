...
using acu::common::util::PairHash;
  ...
  for (auto& road_internal : *roads) {
    ...
    for (auto& traffic_light_internal : road_internal.traffic_lights) {
      ...
      for (auto stop_line_id : traffic_light_internal.stop_line_ids) {
        ACU_CHECK_GT(proto_data_.pb_stop_lines.count(stop_line_id), 0);
...
void ProtoOrganizer::GetRoadsurfaces(
    const std::vector<RoadSurfaceInternal>& roadsurfaces) {
  for (auto& roadsurface : roadsurfaces) {
    //isolationbelt
    for (auto& isolationbelt : roadsurface.isolationbelts) {
      std::string isolation_id = isolationbelt.id().id();
      proto_data_.pb_isolationbelt[isolation_id] = isolationbelt;
    }
    //guardrail
    for (auto& guardrail : roadsurface.guardrails) {
      std::string guardrail_id = guardrail.id().id();
      proto_data_.pb_guardrail[guardrail_id] = guardrail;
    }
    //inner
    for (auto& inner : roadsurface.inners) {
      std::string inner_id = inner.id().id();
      proto_data_.pb_inner[inner_id] = inner;
    }
    //outer
    for (auto& outer : roadsurface.outers) {
      std::string outer_id = outer.id().id();
      proto_data_.pb_outer[outer_id] = outer;
    }
  }
}
...
void ProtoOrganizer::GetLaneObjectOverlapElements(
  ...
  for (auto& overlap_object : overlap_with_lanes) {
    ...
    if (proto_data_.pb_crosswalks.count(object_id) > 0) {
      ...
      std::cout<<"unknown object, object id:" << object_id<<std::endl;
    }
...
void ProtoOrganizer::GetLaneSignalOverlapElements(
  ...
  for (auto& overlap_signal : overlap_with_lanes) {
    ...
    if (proto_data_.pb_signals.count(object_id) <= 0 &&
      ...
      std::cout<<"cannot find signal object_id:" << object_id<<std::endl;
      continue;
    }
    PbOverlap overlap;
    std::string overlap_id = CreateOverlapId();
    ...
    if (proto_data_.pb_signals.count(object_id) > 0) {
      ...
      std::cout<<"unknown signal, signal id:" << object_id<<std::endl;
    }
    proto_data_.pb_overlaps[overlap_id] = overlap;
  }
}
...
void ProtoOrganizer::GetLaneJunctionOverlapElements(
  ...
  for (auto& overlap_junction : overlap_with_lanes) {
    ...
    if (proto_data_.pb_junctions.count(object_id) <= 0) {
      std::cout<< "cannot find junction object " << object_id<<std::endl;
      continue;
    }
    PbOverlap overlap;
    std::string overlap_id = CreateOverlapId();
    ...
    if (proto_data_.pb_junctions.count(object_id) > 0) {
      ...
      std::cout<<"unknown junction overlap, id:" << object_id<<std::endl;
    }
    proto_data_.pb_overlaps[overlap_id] = overlap;
  }
}
...
void ProtoOrganizer::GetLaneLaneOverlapElements(
  ...
  for (auto& overlap_lane_pair : lane_lane_overlaps) {
    ...
    if (proto_data_.pb_lanes.count(object_id) <= 0) {
      std::cout<<"unknown overlap lane, id:" << object_id<<std::endl;
      continue;
    }

    if (lane_lane_overlaps.count(make_pair(object_id, lane_id)) <= 0) {
      std::cout<<"lane overlap is not symmetrical " << overlap_id<<std::endl;
      continue;
    }
...
void ProtoOrganizer::OutputData(acu::hdmap::Map* pb_map) {
  ...
  for (auto& isolationbelt_pair : proto_data_.pb_isolationbelt) {
    *(pb_map->add_isolationbelt()) = isolationbelt_pair.second;
  }
  for (auto& guardrail_pair : proto_data_.pb_guardrail) {
    *(pb_map->add_guardrail()) = guardrail_pair.second;
  }
  for (auto& inner_pair : proto_data_.pb_inner) {
    *(pb_map->add_inner()) = inner_pair.second;
  }
  for (auto& outer_pair : proto_data_.pb_outer) {
    *(pb_map->add_outer()) = outer_pair.second;
  }

  std::cout<<"hdmap statistics: roads-" << proto_data_.pb_roads.size()
        ...
        << proto_data_.pb_junctions.size() << ",isolationbelts-"
        << proto_data_.pb_isolationbelt.size() << ",guardrails-"
        << proto_data_.pb_guardrail.size() << ", inners-"
        << proto_data_.pb_inner.size() << ", outers-"
        << proto_data_.pb_outer.size() << ", overlaps-"
        << proto_data_.pb_overlaps.size()<<std::endl;
}
