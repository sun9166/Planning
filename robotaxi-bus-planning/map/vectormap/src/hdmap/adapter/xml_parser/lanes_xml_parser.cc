...
std::vector<std::string> LanesXmlParser::opposite_ids_;
...
Status LanesXmlParser::Parse(const tinyxml2::XMLElement& xml_node,
  ...
  // opposite_ids_.assign(opposite_ids.begin(), opposite_ids.end());
  while(opposite_ids.size() > 0) {
    opposite_ids_.push_back(opposite_ids[opposite_ids.size() - 1]);
    opposite_ids.pop_back();
  }
  ACU_RETURN_IF_NULL(sections);
  const auto lanes_node = xml_node.FirstChildElement("lanes");
  ACU_RETURN_IF_NULL(lanes_node);
  const tinyxml2::XMLElement* sub_node =
      lanes_node->FirstChildElement("laneSection");
  ACU_RETURN_IF_NULL(sub_node);
...
Status LanesXmlParser::ToPbBoundaryType(const std::string& type,
                                        PbBoundaryEdgeType* boundary_type) {
  ACU_RETURN_IF_NULL(boundary_type);
...
Status LanesXmlParser::ParseLane(const tinyxml2::XMLElement& xml_node,
                                 LaneInternal* lane_internal) {
  ACU_RETURN_IF_NULL(lane_internal);
  ...
  if (sub_node) {
    ...
    ACU_CHECK(lane_boundary != nullptr);
    success =
  ...
  if (sub_node) {
    ParseLaneLink(*sub_node, lane, id);
  }
...
Status LanesXmlParser::ParseRoadSampleAssociates(
  ...
  if (sub_node == nullptr) {
    std::string err_msg = "Error parse road sample associations1";
    // return Status(acu::common::ErrorCode::HDMAP_DATA_ERROR, err_msg);
    return Status::OK(); // some map might not have this tag
  }
  sub_node = sub_node->FirstChildElement("sampleAssociation");
  if (sub_node == nullptr) {
    std::string err_msg = "Error parse road sample association2";
    // return Status(acu::common::ErrorCode::HDMAP_DATA_ERROR, err_msg);
    return Status::OK();
  }
...
Status LanesXmlParser::ParseObjectOverlapGroup(
  ...
  ACU_RETURN_IF_NULL(object_overlaps);
...
Status LanesXmlParser::ParseSignalOverlapGroup(
  ...
  ACU_RETURN_IF_NULL(signal_overlaps);
...
Status LanesXmlParser::ParseJunctionOverlapGroup(
  ...
  ACU_RETURN_IF_NULL(junction_overlaps);
...
Status LanesXmlParser::ParseLaneOverlapGroup(
  ...
  ACU_RETURN_IF_NULL(lane_overlaps);
...
Status LanesXmlParser::ToPbLaneType(const std::string& type,
  ...
  } else if (upper_str == "BUSING") {
    *lane_type = hdmap::Lane::BUSING;
  } else if (upper_str == "ISLAND") {
    *lane_type = hdmap::Lane::ISLAND;
  } else if (upper_str == "WAITINGLEFT") {
    *lane_type = hdmap::Lane::WAITINGLEFT;
  } else if (upper_str == "WAITINGSTRAIGHT") {
    *lane_type = hdmap::Lane::WAITINGSTRAIGHT;
  } else {
...
Status LanesXmlParser::ToPbTurnType(const std::string& type,
                                    PbTurnType* pb_turn_type) {
  ACU_RETURN_IF_NULL(pb_turn_type);
...
Status LanesXmlParser::ToPbDirection(const std::string& type,
  ...
  ACU_RETURN_IF_NULL(pb_direction);
...
void LanesXmlParser::ParseLaneLink(const tinyxml2::XMLElement& xml_node,
  ...
  if (id == -1) {
    while(opposite_ids_.size() > 0) {
      // std::cout << "\t   opposite lane_id: " << opposite_ids_[opposite_ids_.size() - 1] << std::endl;
      lane->add_left_neighbor_reverse_lane_id()->set_id(opposite_ids_[opposite_ids_.size() - 1]);
      opposite_ids_.pop_back();
    }
  }
}
