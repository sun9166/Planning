...
using acu::common::math::AABoxKDTreeParams;
...
int HDMapImpl::LoadMapFromFile(const std::string& map_filename) {
  ...
  if (acu::common::util::EndWith(map_filename, ".xml")) {
    if (!adapter::OpendriveAdapter::LoadData(map_filename, &map_)) {
      return -1;
    }
  } else if (!acu::common::util::GetProtoFromFile(map_filename, &map_)) {
    return -1;
  }
...
int HDMapImpl::LoadMapFromProto(const Map& map_proto) {
  ...
  for (const auto& isolationbelt : map_.isolationbelt()) {
    isolationbelt_table_[isolationbelt.id().id()].reset(
      new IsolationBeltInfo(isolationbelt));
  }
  for (const auto& guardrail : map_.guardrail()) {
    guardrail_table_[guardrail.id().id()].reset(
      new GuardrailInfo(guardrail));
  }
  for (const auto& inner : map_.inner()) {
    inner_table_[inner.id().id()].reset(
      new InnerInfo(inner));
  }
  for (const auto& outer : map_.outer()) {
    outer_table_[outer.id().id()].reset(
      new OuterInfo(outer));
  }
    ...
    for (const auto& road_section : road_ptr_pair.second->sections()) {
      ...
      for (const auto& lane_id : road_section.lane_id()) {
        ...
        if (iter != lane_table_.end()) {
          ...
          AERROR << "Unknown lane id: " << lane_id.id();
        }
      }
    }
  }
  ...
  BuildSpeedBumpPolygonKDTree();
  BuildIsolationBeltPolygonKDTree();
  BuildGuardrailPolygonKDTree();
  BuildInnerPolygonKDTree();
  BuildOuterPolygonKDTree();
...
IsolationBeltInfoConstPtr HDMapImpl::GetIsolationBeltById(const Id& id) const {
  IsolationBeltTable::const_iterator it = isolationbelt_table_.find(id.id());
  return it != isolationbelt_table_.end() ? it->second : nullptr;
}

GuardrailInfoConstPtr HDMapImpl::GetGuardrailById(const Id& id) const {
  GuardrailTable::const_iterator it = guardrail_table_.find(id.id());
  return it != guardrail_table_.end() ? it->second : nullptr;
}

InnerInfoConstPtr HDMapImpl::GetInnerById(const Id& id) const {
  InnerTable::const_iterator it = inner_table_.find(id.id());
  return it != inner_table_.end() ? it->second : nullptr;
}

OuterInfoConstPtr HDMapImpl::GetOuterById(const Id& id) const {
  OuterTable::const_iterator it = outer_table_.find(id.id());
  return it != outer_table_.end() ? it->second : nullptr;
}
...
int HDMapImpl::GetIsolationBelts(
  const acu::common::PointENU& point, double distance,
  std::vector<IsolationBeltInfoConstPtr>* isolationbelts) const {
  return GetIsolationBelts({point.x(), point.y()}, distance, isolationbelts);
}

int HDMapImpl::GetIsolationBelts(
  const acu::common::math::Vec2d& point, double distance,
  std::vector<IsolationBeltInfoConstPtr>* isolationbelts) const {
  if (isolationbelts == nullptr || isolationbelt_polygon_kdtree_ == nullptr) {
    return -1;
  }
  isolationbelts->clear();
  std::vector<std::string> ids;
  const int status =
    SearchObjects(point, distance, *isolationbelt_polygon_kdtree_, &ids);
  if (status < 0) {
    return status;
  }
  for (const auto& id : ids) {
    isolationbelts->emplace_back(GetIsolationBeltById(CreateHDMapId(id)));
  }
  return 0;
}

int HDMapImpl::GetGuardrails(
  const acu::common::PointENU& point, double distance,
  std::vector<GuardrailInfoConstPtr>* guardrails) const {
  return GetGuardrails({point.x(), point.y()}, distance, guardrails);
}

int HDMapImpl::GetGuardrails(
  const acu::common::math::Vec2d& point, double distance,
  std::vector<GuardrailInfoConstPtr>* guardrails) const {
  if (guardrails == nullptr || guardrail_polygon_kdtree_ == nullptr) {
    return -1;
  }
  guardrails->clear();
  std::vector<std::string> ids;
  const int status =
    SearchObjects(point, distance, *guardrail_polygon_kdtree_, &ids);
  if (status < 0) {
    return status;
  }
  for (const auto& id : ids) {
    guardrails->emplace_back(GetGuardrailById(CreateHDMapId(id)));
  }
  return 0;
}

int HDMapImpl::GetInners(
  const acu::common::PointENU& point, double distance,
  std::vector<InnerInfoConstPtr>* inners) const {
  return GetInners({point.x(), point.y()}, distance, inners);
}

int HDMapImpl::GetInners(
  const acu::common::math::Vec2d& point, double distance,
  std::vector<InnerInfoConstPtr>* inners) const {
  if (inners == nullptr || inner_polygon_kdtree_ == nullptr) {
    return -1;
  }
  inners->clear();
  std::vector<std::string> ids;
  const int status =
    SearchObjects(point, distance, *inner_polygon_kdtree_, &ids);
  if (status < 0) {
    return status;
  }
  for (const auto& id : ids) {
    inners->emplace_back(GetInnerById(CreateHDMapId(id)));
  }
  return 0;
}

int HDMapImpl::GetOuters(
  const acu::common::PointENU& point, double distance,
  std::vector<OuterInfoConstPtr>* outers) const {
  return GetOuters({point.x(), point.y()}, distance, outers);
}

int HDMapImpl::GetOuters(
  const acu::common::math::Vec2d& point, double distance,
  std::vector<OuterInfoConstPtr>* outers) const {
  if (outers == nullptr || outer_polygon_kdtree_ == nullptr) {
    return -1;
  }
  outers->clear();
  std::vector<std::string> ids;
  const int status =
    SearchObjects(point, distance, *outer_polygon_kdtree_, &ids);
  if (status < 0) {
    return status;
  }
  for (const auto& id : ids) {
    outers->emplace_back(GetOuterById(CreateHDMapId(id)));
  }
  return 0;
}
...
int HDMapImpl::GetSpeedBumps(
  const Vec2d& point, double distance,
  std::vector<SpeedBumpInfoConstPtr>* speed_bumps) const {
  if (speed_bumps == nullptr || speed_bump_polygon_kdtree_ == nullptr) {
    return -1;
  }
  speed_bumps->clear();
  std::vector<std::string> ids;
  const int status =
    SearchObjects(point, distance, *speed_bump_polygon_kdtree_, &ids);
  if (status < 0) {
    return status;
  }
  for (const auto& id : ids) {
...
int HDMapImpl::GetNearestLane(const Vec2d& point,
                              LaneInfoConstPtr* nearest_lane, double* nearest_s,
                              double* nearest_l) const {
  ACU_RETURN_IF_NULL(nearest_lane);
  ACU_RETURN_IF_NULL(nearest_s);
  ACU_RETURN_IF_NULL(nearest_l);
  ...
  *nearest_lane = GetLaneById(lane_id);
  ACU_CHECK(*nearest_lane);
  const int id = segment_object->id();
  if (id < 0 || id >= (*nearest_lane)->segments().size() || 
      id >= (*nearest_lane)->accumulate_s().size()) {
  ...
  *nearest_l = segment.unit_direction().CrossProd(point - segment.start());
  //if (fabs(*nearest_l) > 3.5) {
    //ROS_ERROR_STREAM("nearest_s is "<<*nearest_s<<" nearest_l is "<<*nearest_l);
    // return -1;
  //}
  return 0;
}
...
int HDMapImpl::GetNearestLaneWithHeading(
  ...
  for (const auto& lane : lanes) {
    // if (lane == nullptr || !lane->lane().has_type()) {
    if (lane == nullptr) {
      // AERROR<<"lane ptr is "<<lane->id().id()<<" lane->lane().has_type() "<<lane->lane().has_type();
      continue;
    }
    if (lane->lane().type() == acu::hdmap::Lane_LaneType::Lane_LaneType_SIDEWALK) {
      // ROS_WARN_STREAM(lane->id().id()<<" is not CITY_DRIVING");
      continue;
    }
    double s_offset = 0.0;
...
int HDMapImpl::GetNearestCruiseLaneWithHeading(const acu::common::PointENU& point,
                                const double distance,
                                const double central_heading,
                                const double max_heading_difference,
                                LaneInfoConstPtr* nearest_lane,
                                double* nearest_s, double* nearest_l) const {
  return GetNearestCruiseLaneWithHeading({point.x(), point.y()}, distance,
                                   central_heading, max_heading_difference,
                                   nearest_lane, nearest_s, nearest_l);
}

int HDMapImpl::GetNearestCruiseLaneWithHeading(const acu::common::math::Vec2d& point,
                                const double distance,
                                const double central_heading,
                                const double max_heading_difference,
                                LaneInfoConstPtr* nearest_lane,
...
int HDMapImpl::GetLanesWithHeading(const PointENU& point, const double distance,
  ...
  return GetLanesWithHeading({point.x(), point.y()}, distance, central_heading,
                             max_heading_difference, lanes, distances);
}
...
int HDMapImpl::GetLanesWithHeading(const Vec2d& point, const double distance,
  ...
  for (auto& lane : all_lanes) {
    ...
    if (dis <= distance) {
      ...
      if (fabs(acu::common::math::NormalizeAngle(heading_diff)) <=
...
int HDMapImpl::GetRoadBoundaries(
  ...
  //ACU_RETURN_IF_NULL(road_boundaries);
  //ACU_RETURN_IF_NULL(junctions);
  ...
  for (const auto& lane : lanes) {
    ...
    if (road_ptr->has_junction_id()) {
      ...
      //ACU_RETURN_IF_NULL(junction_boundary_ptr->junction_info);
...
int HDMapImpl::GetForwardNearestSignalsOnLane(
  ...
  while (s < back_distance) {
    for (const auto& predecessor_lane_id : lane_ptr->lane().predecessor_id()) {
      ...
      if (lane_ptr->lane().turn() == acu::hdmap::Lane::NO_TURN) {
        break;
      }
    }
  ...
  while (lane_ptr != nullptr) {
    ...
    for (const auto& successor_lane_id : lane_ptr->lane().successor_id()) {
      ...
      if (tmp_lane_ptr->lane().turn() == acu::hdmap::Lane::NO_TURN) {
        break;
      }
    }
  ...
  for (const auto& info_with_id : table) {
    ...
    for (size_t id = 0; id < info->segments().size(); ++id) {
      ...
      box_table->emplace_back(
        acu::common::math::AABox2d(segment.start(), segment.end()), info,
        &segment, id);
    }
  }
...
void HDMapImpl::BuildIsolationBeltPolygonKDTree() {
  AABoxKDTreeParams params;
  params.max_leaf_dimension = 5.0;  // meters.
  params.max_leaf_size = 4;
  BuildPolygonKDTree(isolationbelt_table_, params, &isolationbelt_polygon_boxes_,
                     &isolationbelt_polygon_kdtree_);
}

void HDMapImpl::BuildGuardrailPolygonKDTree() {
  AABoxKDTreeParams params;
  params.max_leaf_dimension = 5.0;  // meters.
  params.max_leaf_size = 4;
  BuildPolygonKDTree(guardrail_table_, params, &guardrail_polygon_boxes_,
                     &guardrail_polygon_kdtree_);
}

void HDMapImpl::BuildInnerPolygonKDTree() {
  AABoxKDTreeParams params;
  params.max_leaf_dimension = 5.0;  // meters.
  params.max_leaf_size = 4;
  BuildPolygonKDTree(inner_table_, params, &inner_polygon_boxes_,
                     &inner_polygon_kdtree_);
}

void HDMapImpl::BuildOuterPolygonKDTree() {
  AABoxKDTreeParams params;
  params.max_leaf_dimension = 5.0;  // meters.
  params.max_leaf_size = 4;
  BuildPolygonKDTree(outer_table_, params, &outer_polygon_boxes_,
                     &outer_polygon_kdtree_);
}
...
void HDMapImpl::BuildSpeedBumpPolygonKDTree() {
  AABoxKDTreeParams params;
  params.max_leaf_dimension = 5.0;  // meters.
  params.max_leaf_size = 4;
  BuildPolygonKDTree(speed_bump_table_, params, &speed_bump_polygon_boxes_,
                     &speed_bump_polygon_kdtree_);
}
...
void HDMapImpl::Clear() {
  ...
  speed_bump_polygon_boxes_.clear();
  speed_bump_polygon_kdtree_.reset(nullptr);
}
