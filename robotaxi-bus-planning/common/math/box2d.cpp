...
void Box2d::GetAllCorners(std::vector<Vec2d> *const corners) const {
  ...
  corners->clear();
  corners->reserve(4);
  corners->emplace_back(center_.x() + dx1 + dx2, center_.y() + dy1 + dy2);//右前
  corners->emplace_back(center_.x() + dx1 - dx2, center_.y() + dy1 - dy2);//左前
  corners->emplace_back(center_.x() - dx1 - dx2, center_.y() - dy1 - dy2);//左后
  corners->emplace_back(center_.x() - dx1 + dx2, center_.y() - dy1 + dy2);//右后
}
...
double Box2d::DistanceToBoundary(const Vec2d &point) const {
  const double x0 = point.x() - center_.x();
  const double y0 = point.y() - center_.y();
  const double dx =
    std::abs(x0 * cos_heading_ + y0 * sin_heading_) - half_length_;
