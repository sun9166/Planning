...
bool Polygon2d::VerticalDistanceTo(const double &s, std::pair<double, double>& l_range) const {
  l_range.first = 100.0;
  l_range.second = -100.0;
  bool find_flag = false;
  for (auto &edge : line_segments_) {
    // std::cout<<"edge.start().x ="<<edge.start().x()<<", "<<edge.end().x()<<std::endl;
    if (s <= fmin(edge.start().x(), edge.end().x()) ||
        s >= fmax(edge.start().x(), edge.end().x()) ) {
      continue;
    }
    find_flag = true;
    Vec2d start(s, -100.0), end(s, 100.0), intersection(100.0, -100.0);
    LineSegment2d s_line(start, end);
    if (edge.GetIntersect(s_line, &intersection)) {
      l_range.first = fmin(l_range.first, intersection.y());
      l_range.second = fmax(l_range.second, intersection.y());
    }
  }
  if (!find_flag) {
    return false;
  }
  if (l_range.second - l_range.first < 0.1) {
    l_range.second = l_range.first + 0.1;
  }
  return (l_range.second > l_range.first);
}
